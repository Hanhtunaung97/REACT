...
const List = ({ id, job, isDone, deleteTask, doneTask, updateTask }) => {
  ...
  return (
    <div className="list">
      <div className="group animate__animated animate__bounceIn border mb-3 overflow-hidden border-neutral-700 p-5 flex justify-between items-center">
        {edit ? (
          <div>
            <input
              ...
              name="updated_text"
              id="updateInput"
              className="border border-neutral-500 px-2 py-1 text-sm w-[280px] focus:outline-neutral-300 "
            />
          </div>
        ) : (
        ...
        <div className="control opacity-100 pointer-events-none duration-300 translate-x-[100px] group-hover:pointer-events-auto group-hover:opacity-100 group-hover:translate-x-0 flex gap-1">
          <button
            ...
            className={`list-edit duration-300 active:scale-75 disabled:opacity-20 ${
              isDone && "opacity-20 pointer-events-none select-none"
            }`}
          >
            <svg
